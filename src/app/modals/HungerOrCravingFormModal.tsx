...
interface HungerOrCravingFormModalProps {
  show: boolean;
  title: string;
...
const HungerOrCravingFormModal: React.FC<HungerOrCravingFormModalProps> = ({
  show,
  title,
  data,
  handleClose,
  ...
  const [hungerOrCraving, setHungerOrCraving] = useState<IDefaultOptionsData | null>(null);
  const [validation, setValidation] = useState<DefaultOptionsValidateData>({
    option: false,
  });
  useEffect(() => {
    if (title === 'Edit' && data) {
      setHungerOrCraving(data);
    } else {
      setHungerOrCraving(null)
    }
  }, [data, title]);
  const handleChange = (value: string, name: string) => {
    const temp: any = { ...hungerOrCraving };
    ...
    temp[name] = value;
    setHungerOrCraving(temp);
    setValidation(tempValidation);
  };
  const handleSubmit = async () => {
    const temp = { ...hungerOrCraving };
    ...
    if (!tempValidation.option) {
      ...
      if (response) {
        if (title === 'Add') {
          success(defaultOptionsToast.hungerOrCravingAdded);
        } else if (title === 'Edit') {
          success(defaultOptionsToast.hungerOrCravingUpdated);
        }
        onHide();
      }
      setLoading(false);
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title className="fs-22 fw-bolder">
          {title === 'Add' ? defaultOptionsString.addHungerOrCraving : defaultOptionsString.editHungerOrCraving}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
export default HungerOrCravingFormModal;
