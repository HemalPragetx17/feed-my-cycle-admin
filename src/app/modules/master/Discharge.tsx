...
import DischargeFormModal from "../../modals/DischargeFormModal";
const options = [
  {
    label: (
      <button className="btn btn-link fs-15 fw-600 text-black ms-3 p-4">
        {defaultOptionsString.editDischarge}
      </button>
    ),
    value: 1,
  },
  {
    label: (
      <button className="btn btn-link fs-15 fw-600 text-danger ms-3 p-4">
        {defaultOptionsString.deleteDischarge}
      </button>
    ),
    value: 2,
  },
...
const Discharge = () => {
  const [dischargesData, setDischargesData] = useState<IDefaultOptionsData[]>([]);
  ...
  // Modal states
  const [dischargeModal, setShowDischargeModal] = useState<boolean>(false);
  const [dischargeModalTitle, setDischargeModalTitle] = useState<string>('');
  const [currentDischarge, setCurrentDischarge] = useState<IDefaultOptionsData | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      await fetchDischarges(1, pageLimit, search);
      setFetchLoading(false);
    })();
  }, []);

  const fetchDischarges = async (pageNo: number, limit: number, search: string = '') => {
    setLoading(true);
    const params = {
      page: pageNo,
      limit: limit,
    ...
    if (response && response.records) {
      ...
      setDischargesData(response.records);
    } else {
      setDischargesData([]);
    }
    setLoading(false);
  ...
  const handleDeleteConfirm = async () => {
    const apiService = new APICallService(DEFAULT_OPTIONS.DELETE_DEFAULT_OPTION, currentDischarge?._id);
    const response = await apiService.callAPI();
    if (response) {
      success(defaultOptionsToast.dischargeDeleted);
  ...
  const handleCurrentPage = async (val: number) => {
    ...
    setPage(val);
    await fetchDischarges(val, pageLimit, search);
  };

  const handleNextPage = async (val: number) => {
    setPage(val + 1);
    await fetchDischarges(val + 1, pageLimit, search);
  };
  ...
  const handlePageLimit = async (event: any) => {
    ...
    await fetchDischarges(1, event.target.value, search);
  };

  const debounce = useDebounce(fetchDischarges, 300);
  ...
  return (
    <>
      <Row className="align-items-center g-md-2 g-3 mb-7">
        <Col sm>
          <h1 className="fs-22 fw-bolder">{defaultOptionsString.discharge}</h1>
        </Col>
        {!fetchLoading ? (
          <Col md={'auto'} className="text-right mb-5">
            <Button
              onClick={() => {
                ...
                setDischargeModalTitle('Add');
              }}
              className="btn btn-primary mh-md-50px btn-lg fs-16 fw-600"
            >
              {defaultOptionsString.addDischarge}
            </Button>
          </Col>
        ) : (
          <></>
              ...
              <Row className="align-items-center">
                <Col xl={6} lg={6} md={12} className="mb-xl-0 mb-5">
                  <div className="form-group">
                    <Form.Label className="fs-16 fw-500">Search Discharges</Form.Label>
          ...
          <Card.Body className="p-0">
            <div className="table-responsive">
              <table className="table table-rounded table-row-bordered align-middle gs-7 gy-4 mb-0">
                ...
                <tbody>
                  {dischargesData && dischargesData.length ? (
                    dischargesData.map((item) => (
                      <tr key={item._id}>
                        <td>
                          {item.option}
                        </td>
                        <td className="text-center">
                          <CustomSelectTable
                            ...
                            onChange={(event: any) => {
                              if (event.value === 1) {
                                setCurrentDischarge(item);
                                setShowDischargeModal(true);
                                setDischargeModalTitle('Edit');
      ...
      {!loading && totalRecords > 0 && dischargesData.length > 0 && (
        <Pagination
      ...
      <CommonDeleteModal
        ...
        title={defaultOptionsString.deleteDischarge}
        id={currentDischarge?._id || ''}
        btnText={"Delete"}
        headerText="this Discharge"
