...
import CycleStageFormModal from "../../modals/CycleStageFormModal";
const options = [
  {
    label: (
      <button className="btn btn-link fs-15 fw-600 text-black ms-3 p-4">
        {defaultOptionsString.editCycleStage}
      </button>
    ),
    value: 1,
  },
  {
    label: (
      <button className="btn btn-link fs-15 fw-600 text-danger ms-3 p-4">
        {defaultOptionsString.deleteCycleStage}
      </button>
    ),
    value: 2,
  },
...
const CycleStages = () => {
  const [cycleStagesData, setCycleStagesData] = useState<IDefaultOptionsData[]>([]);
  ...
  // Modal states
  const [cycleStagesModal, setShowCycleStagesModal] = useState<boolean>(false);
  const [cycleStagesModalTitle, setCycleStagesModalTitle] = useState<string>('');
  const [currentCycleStage, setCurrentCycleStage] = useState<IDefaultOptionsData | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      await fetchCycleStages(1, pageLimit, search);
      setFetchLoading(false);
    })();
  }, []);

  const fetchCycleStages = async (pageNo: number, limit: number, search: string = '') => {
    setLoading(true);
    const params = {
      pageNo: pageNo,
      limit: limit,
    ...
    if (response && response.records) {
      ...
      setCycleStagesData(response.records);
    } else {
      setCycleStagesData([]);
    }
    setLoading(false);
  ...
  const handleDeleteConfirm = async () => {
    const apiService = new APICallService(DEFAULT_OPTIONS.DELETE_DEFAULT_OPTION, currentCycleStage?._id);
    const response = await apiService.callAPI();
    if (response) {
      success(defaultOptionsToast.cycleStageDeleted);
      setCurrentCycleStage(null);
  ...
  const handleCurrentPage = async (val: number) => {
    ...
    setPage(val);
    await fetchCycleStages(val, pageLimit, search);
  };

  const handleNextPage = async (val: number) => {
    setPage(val + 1);
    await fetchCycleStages(val + 1, pageLimit, search);
  };
  ...
  const handlePageLimit = async (event: any) => {
    ...
    await fetchCycleStages(1, event.target.value, search);
  };

  const debounce = useDebounce(fetchCycleStages, 300);
  ...
  return (
    <>
      <Row className="align-items-center g-md-2 g-3 mb-7">
        <Col sm>
          <h1 className="fs-22 fw-bolder">{defaultOptionsString.cycleStages}</h1>
        </Col>
        {!fetchLoading ? (
          <Col md={'auto'} className="text-right mb-5">
            <Button
              onClick={() => {
                setShowCycleStagesModal(true);
                setCycleStagesModalTitle('Add');
              }}
              className="btn btn-primary mh-md-50px btn-lg fs-16 fw-600"
            >
              {defaultOptionsString.addCycleStage}
            </Button>
          </Col>
        ) : (
          <></>
              ...
              <Row className="align-items-center">
                <Col xl={6} lg={6} md={12} className="mb-xl-0 mb-5">
                  <div className="form-group">
                    <Form.Label className="fs-16 fw-500">Search Cycle Stages</Form.Label>
          ...
          <Card.Body className="p-0">
            <div className="table-responsive">
              <table className="table table-rounded table-row-bordered align-middle gs-7 gy-4 mb-0">
                ...
                <tbody>
                  {cycleStagesData && cycleStagesData.length ? (
                    cycleStagesData.map((item) => (
                      <tr key={item._id}>
                        <td>
                          {item.option}
                        </td>
                        <td className="text-center">
                          <CustomSelectTable
                            ...
                            onChange={(event: any) => {
                              if (event.value === 1) {
                                setCurrentCycleStage(item);
                                setShowCycleStagesModal(true);
                                setCycleStagesModalTitle('Edit');
      ...
      {!loading && totalRecords > 0 && cycleStagesData.length > 0 && (
        <Pagination
      ...
      <CycleStageFormModal
        show={cycleStagesModal}
        title={cycleStagesModalTitle}
        data={currentCycleStage}
        handleClose={() => {
          setShowCycleStagesModal(false);
          setCurrentCycleStage(null);
        }}
        onHide={async () => {
          setShowCycleStagesModal(false);
      ...
      <CommonDeleteModal
        ...
        title={defaultOptionsString.deleteCycleStage}
        id={currentCycleStage?._id || ''}
        btnText={"Delete"}
        headerText="this Cycle Stage"
