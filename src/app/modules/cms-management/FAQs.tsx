...
import { IFAQData } from "../../../types/request_data/faqs";
...
const FAQs = () => {
  ...
  // Modal states
  const [faqModal, setShowFaqModal] = useState<boolean>(false);
  const [faqModalTitle, setFaqModalTitle] = useState<string>('');
  const [currentFaq, setCurrentFaq] = useState<IFAQData | null>(null);
  ...
  const fetchFaq = async (pageNo: number, limit: number, search: string = '') => {
    setLoading(true);
    const params = {
      page: pageNo,
      limit: limit,
  ...
  const handleDeleteConfirm = async () => {
    const apiService = new APICallService(CMS.DELETE_FAQ, currentFaq?._id);
    const response = await apiService.callAPI();
    if (response) {
  ...
  return (
    <>
            ...
            <Row className="align-items-center">
              <Col xl={6} lg={6} md={12} className="mb-xl-0 mb-5">
                <div className="form-group">
                  <Form.Label className="fs-16 fw-500">Search FAQ</Form.Label>
              ...
              <table className="table table-rounded table-row-bordered align-middle gs-7 gy-4 mb-0">
                <thead>
                  <tr className="fw-bold fs-16 fw-600 text-dark border-bottom h-70px align-middle gy-3">
                    <th style={{ minWidth: 200 }}>{faqString.question}</th>
                    <th style={{ minWidth: 300 }}>{faqString.answer}</th>
      ...
      {!loading && totalRecords > 0 && faqsData.length > 0 && (
        <Pagination
